...
(** Функциональный стиль программирования основан на простой, 
    ...
    абстракции и повторное использование кода. Coq предоставляет все 
	эти возможности.
    ...
    используемые для доказательства свойств программ в Coq. *)
...
(** Это определение делает две вещи: утверждает, что второй следующий 
    ...
    дальнейшем. Сделав утверждение, мы можем попросить Coq проверить
    его, например, так: *)
...
(** Подробности сейчас не важны (мы вернемся к этому через некоторое
    ...
    перейти от доказано корректных алгоритмов, написанных на языке
...
(** Мы можем ввести знакомый синтаксис для операций, которые только
    что определили. Команда [Notation] задает новую нотацию для
...
(** Тип выражения [negb], записываемый как [bool -> bool], можно 
    ...
    возвращает значение типа [bool]". Похожим образом, тип значения
...
(** Давайте посмотрим внимательнее.
    ...
    Каждый заданный индуктивно тип ([day], [bool], [rgb], [color], 
...
(** (Примечания [level (уровень)], [associativity (ассоциативность)] и
    ...
    мы просто инструктируем Coq, что он должен принимать [x + y] 
...
(** Когда мы говорили, что в Coq нет почти ничего встроенного, мы не
    ...
    где результатом является значение логического типа ([b-oolean]).
...
(** Теперь, когда мы определили несколько типов данных и функций,
    ...
    [reflexivity], чтобы проверить, что значения по обе стороны от 
    равенства идентичны.
...
(** (Если вы посмотрите на исходный код в файле [.v] и на HTML версию
    ...
    этот квантор превращается в перевернутую букву А.)
...
(** Более того, будет полезным знать, что [reflexivity] выполняет
    ...
    "разворачивать" определения, заменяя их имена правыми частями
...
(** Первая строка доказательства перемещает переменные [n] и [m],
    находящиеся под квантором всеобщности, в контекст. Вторая 
...
(** Команда [Admitted] сообщает Coq, что мы хотим пропустить 
    ...
    доказательства. Однако, будьте осторожны: каждый раз, когда вы
...
(** Разумеется, не все можно доказать вычислениями и переписываниями:
    ...
    блокировать вычисления. Например, если мы попробуем доказать 
...
(** Тактика [destruct] порождает _две_ подцели, которые мы затем должны
    доказать отдельно, чтобы Coq принял нашу теорему. Примечание
    "[as [| n']]" называется _образцом введения_. Он говорит Coq, какие
    ...
    проблемы особенно актуальны в больших разработках, где "хрупкие",
...
(** Заметьте, что команда [destruct] здесь не имеет соответствующего
    ...
    не очевидные имена, когда предоставлен сам себе.
...
(** Для каждой нотации в Coq мы можем задать _уровень приоритета_
    ...
    Каждая нотация связана с _контекстом_ или _областью действия_. 
    ...
    в следующих главах) -- с контекстом типов [type_scope]. Время от
    ...
    а иногда -- [0%Z], что означает ноль в целых числах (которые 
...
(** Когда Coq проверяет это определение, он видит, что [plus']
    ...
    _структурную рекурсию_ по аргументу [n], то есть мы делаем 
...
(** Обдумайте другое, более эффективное представление натуральных
    чисел, использующее двоичную систему счисления. Таким образом,
    ...
    (a) Во-первых, запишите индуктивное определение типа [bin],
        ...
        определение [bin] должно быть таким же простым. Математический
